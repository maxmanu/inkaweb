<?php include("template-parts/header.php") ?>

<!--breadcrumb area start-->
<section class="breadcrumb-area bg-primary-gradient">
  <div class="container">
    <div class="breadcrumb-content text-center">
      <h2 class="mb-3">Categoría de Tutorial</h2>
    </div>
  </div>
</section>
<!--breadcrumb area end-->

<!--blog grid start-->
<section class="hm-blog-grids pt-120 pb-120 overflow-hidden">
  <div class="container">
    <div class="row g-5">
      <div class="col-xl-8">
        <div class="hm-blog-grid-left">
          <div class="row g-4">
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-3.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">Improving Website Performance with LiteSpeed.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-3.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">The definitive list of digital products you can sell.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-3.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">Improving Website Performance with LiteSpeed.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-4.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">Improving Website Performance with LiteSpeed.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-5.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">Improving Website Performance with LiteSpeed.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-6.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">Improving Website Performance with LiteSpeed.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-7.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">Improving Website Performance with LiteSpeed.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="hm2-blog-card bg-white deep-shadow">
                <div class="feature-img rounded-top overflow-hidden">
                  <a href="tutorial-single.php"><img src="assets/img/blog-grid-8.jpg" alt="feature" class="img-fluid" /></a>
                </div>
                <div class="hm2-blog-card-content position-relative">
                  <a href="#" class="tag-btn rounded-pill position-absolute">Cloud Hosting</a>
                  <a href="tutorial-single.php">
                    <h3 class="h5 mb-3">Improving Website Performance with LiteSpeed.</h3>
                  </a>
                  <p>Many desktop publishing packages & will uncover many web sites.</p>
                  <hr class="spacer mt-20 mb-20" />
                  <div class="bog-author d-flex align-items-center justify-content-between">
                    <span class="date">2 Days Ago</span>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="template-pagination mt-60">
            <ul class="d-flex align-items-center">
              <li><a href="#" class="active">01</a></li>
              <li><a href="#">02</a></li>
              <li><a href="#">03</a></li>
              <li>
                <a href="#"><i class="fa-solid fa-arrow-right"></i></a>
              </li>
            </ul>
          </div>
        </div>
      </div>
      <div class="col-xl-4">
        <div class="hm-blog-sidebar mt-4 mt-xl-0">
          <div class="sidebar-widget search-widget">
            <form>
              <input type="text" placeholder="Palabra Clave...">
              <button class="primary-btn submit-btn"><i class="fa-solid fa-magnifying-glass"></i></button>
            </form>
          </div>
          <div class="sidebar-widget categories-widget">
            <h4 class="widget-title">Categorías</h4>
            <ul class="mt-30">
              <li><a href="tutorial-category.php">Cloud Hosting<sup>(05)</sup></a></li>
              <li><a href="tutorial-category.php">Dedicated Hosting<sup>(03)</sup></a></li>
              <li><a href="tutorial-category.php">WordPress Hosting<sup>(06)</sup></a></li>
              <li><a href="tutorial-category.php">Reseller Hosting<sup>(07)</sup></a></li>
              <li><a href="tutorial-category.php">Shared Hosting<sup>(08)</sup></a></li>
              <li><a href="tutorial-category.php">VPS Hosting<sup>(09)</sup></a></li>
            </ul>
          </div>
          <div class="sidebar-widget recent-post-widget">
            <h4 class="widget-title">Últimos Tutoriales</h4>
            <ul class="mt-30 rs-news-list">
              <li class="d-flex align-items-center">
                <img src="assets/img/rs-blog-1.png" alt="not found" class="img-fluid flex-shrink-0 rounded-circle">
                <div class="rs-news-content ms-3">
                  <a href="tutorial-single.php">
                    <h6 class="mb-0">Improving Website Performance with Lite Speed.</h6>
                  </a>
                </div>
              </li>
              <li class="d-flex align-items-center">
                <img src="assets/img/rs-blog-2.png" alt="not found" class="img-fluid flex-shrink-0 rounded-circle">
                <div class="rs-news-content ms-3">
                  <a href="tutorial-single.php">
                    <h6 class="mb-0">The Definitive list of digital with Products you can Sell.</h6>
                  </a>
                </div>
              </li>
              <li class="d-flex align-items-center">
                <img src="assets/img/rs-blog-3.png" alt="not found" class="img-fluid flex-shrink-0 rounded-circle">
                <div class="rs-news-content ms-3">
                  <a href="tutorial-single.php">
                    <h6 class="mb-0">25 Best Hosting Company All Over the World.</h6>
                  </a>
                </div>
              </li>
            </ul>
          </div>
          <div class="sidebar-widget recent-post-widget">
            <h4 class="widget-title">Links de Importancia</h4>
            <ul class="mt-30 rs-news-list">
              <li class="d-flex align-items-center">
                <div class="rs-news-content ms-3">
                  <a href="tutorial-single.php">
                    <h6 class="mb-0">Improving Website Performance with Lite Speed.</h6>
                  </a>
                </div>
              </li>
              <li class="d-flex align-items-center">
                <div class="rs-news-content ms-3">
                  <a href="tutorial-single.php">
                    <h6 class="mb-0">The Definitive list of digital with Products you can Sell.</h6>
                  </a>
                </div>
              </li>
              <li class="d-flex align-items-center">
                <div class="rs-news-content ms-3">
                  <a href="tutorial-single.php">
                    <h6 class="mb-0">25 Best Hosting Company All Over the World.</h6>
                  </a>
                </div>
              </li>
            </ul>
          </div>
          <div class="sidebar-widget banner-widget">
            <div class="banner-content bg-primary-gradient rounded-2 text-center position-relative zindex-1 overflow-hidden">
              <img src="assets/img/shapes/sidebar-bg.png" alt="net" class="position-absolute left-top soft-light w-100 opacity-50">
              <a href="index.php" class="logo-black"><img src="assets/img/imagenes-inkaweb/logo-inkaweb-dark.svg" alt="logo" class="logo"></a>
              <h6 class="text-white mt-3">Inkawebdesign.com es parte de Doctor Pc Perú E.I.R.L. más de 10 Años de solidez, y una amplia cartera de clientes nos respalda.
              </h6>
              <a href="contacto.php" class="template-btn primary-btn rounded-pill mt-20">Contactar</a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<?php include("template-parts/footer.php") ?>